...
//Сортирање е процес на организирање елементи во низа по одреден редослед.
//Најпознат алгоритам за почетници — СОРТИРАЊЕ СО ПУБЛИКАЦИЈА (Bubble Sort)(најлесен за користење)

void bubbleSort(int a[], int n) {
    int i, j, pom;

    // i ја означува големината на делот од низата што сè уште треба да се сортира
    for(i = n - 1; i > 0; i--) {
        // поместување по низата од почеток до i
        for(j = 0; j < i; j++) {
            if(a[j] > a[j + 1]) {  // ако елементот е поголем од следниот
